...
class SuperRest extends \akou\RestController
{
	...
	function getGenericGetConstraintsString($table_name,$extra_constraints=array())
	{
		...
		$join_string = $this->is_debug ? ' AND '.PHP_EOL : ' AND ';

		return count( $all_constraints ) > 0 ? join( $join_string, $all_constraints ) : '1';
	}

	function logBigString($string)
	{
	...
	function getResultGeneric($table_name,$extra_constraints=array(),$extra_joins='',$extra_sort=array(),$extra_fields=array(),$group_by='',$sort_array=[],$having_constraints = array())
	{
		$class_name = "APP\\$table_name";


		$_all_arguments = func_get_args();
		...
		$hc_boolean =  empty( $having_constraints );
		//error_log('Verify '.print_r( $hc_boolean, true).' '.print_r($having_constraints,true));
		//error_log('GRG2'.print_r($having_constraints,true));

		$having_constraints_str = empty( $having_constraints ) ? '' : ' HAVING '.join(' AND ',$having_constraints);

		//error_log('GRG2 having string '.$having_constraints_str);

		$sql	= 'SELECT DISTINCT SQL_CALC_FOUND_ROWS `'.$table_name.'`.*'.$extra_fields_str.'
			FROM `'.$table_name.'`
			'.PHP_EOL.$extra_joins.PHP_EOL.'
			WHERE '.$constraints_str.PHP_EOL.'
			'.$group_by_str.PHP_EOL.'
			'.$having_constraints_str.PHP_EOL.'
			'.$sort_string.PHP_EOL.'
			LIMIT '.$pagination->limit.PHP_EOL.' '.$offset_string;


		if( $this->is_debug )
		{
			error_log('GENERIC SQL '.$sql );
			$this->logBigString( $sql );
		}
	...
	function genericGet($table_name,$extra_constraints=array(),$extra_joins='',$extra_sort=array(),$extra_fields=array(),$group_by='',$sort_array=array(),$having_constraints=array())
	{
		//if( $this->is_debug )
		//	error_log('Generic get 1'.print_r($having_constraints,true));

		$result = $this->getResultGeneric
		(
			$table_name,
			$extra_constraints,
			$extra_joins,
	...
	function genericPut($table_name,$insert_with_ids=false, $optional_values=array(),$system_values=array(),$banned_values=array())
	{
		$this->setAllowHeader();
		$params = $this->getMethodParams();
		app::connect();
		...
		try
		{
			...
			$to_update_array = $is_assoc ? array( $params ) : $params;

			$result		= $this->genericUpdate( $to_update_array, $table_name, $insert_with_ids, $optional_values,$system_values,$banned_values);
	...
	function genericInsert($array, $table_name, $optional_values=array(), $system_values=array(), $banned_values=null)
	{
		...
		if( $except == $banned_values )
			$except = array('id','created','updated','updated_by_user_id','created_by_user_id');
		...
		foreach($array as $index=>$params )
		{
			...
			else
			{
				...
				else
				{
					...
					if( !$obj_inst->insert() )
					{
						error_log(print_r( $obj_inst->toArray(),true ));
						throw new ValidationException('An error Ocurred please try again later'.$obj_inst->getError() );
					}
